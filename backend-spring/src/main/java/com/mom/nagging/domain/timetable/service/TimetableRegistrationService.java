...
public class TimetableRegistrationService {
    ...
    public List<TimeData> processAndSaveTimetable(byte[] imageBytes, Member member) {
        try {
            ...
            TimetableDto responseDto = objectMapper.readValue(jsonResult, TimetableDto.class);

            // 3. 부모 엔티티인 TimeTable 생성 및 회원(Member) 연결
            TimeTable timeTable = TimeTable.builder()
                    .member(member)
                    .title("내 시간표") // 필요에 따라 이름 동적 설정 가능
                    .build();

            // 4. DTO -> TimeData 엔티티 변환 및 부모(TimeTable) 연결
            List<TimeData> timeDataList = responseDto.data().stream()
                    .map(item -> TimeData.builder()
                            .timeTable(timeTable)
